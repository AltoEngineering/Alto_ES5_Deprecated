 ...
 * @version   1.5.0-beta.5+pre.7af8beb0
 */
...
(function() {
    ...
    if ('undefined' === typeof Alto) {
        ...
        if ('undefined' !== typeof window) {
            window.Alto = window.Alto = Alto = Alto;
        }
    }
...
//
// This is duplicated here to ensure that `Alto.ENV`
// is setup even if `Alto` is not loaded yet.
    if (Alto.ENV) {
    ...
    if (!('MANDATORY_SETTER' in Alto.ENV)) {
        Alto.ENV.MANDATORY_SETTER = true; // default to true for debug dist
    }

    /**
     Define an assertion that will throw an exception if the condition is not
     met. Alto build tools will remove any calls to `Alto.assert()` when
     doing a production build. Example:

     ```javascript
     // Test for truthiness
     Alto.assert('Must pass a valid object', obj);
     // Fail unconditionally
     Alto.assert('This code path should never be run')
     ```
     ...
     */
    Alto.assert = function(desc, test) {
        if (!test) {
            throw new Alto.Error("Assertion Failed: " + desc);
        }
    };


    /**
     Display a warning with the provided message. Alto build tools will
     remove any calls to `Alto.warn()` when doing a production build.
     ...
     will be displayed.
     */
    Alto.warn = function(message, test) {
        if (!test) {
            Alto.Logger.warn("WARNING: "+message);
            if ('trace' in Alto.Logger) Alto.Logger.trace();
        }
    };

    /**
     Display a debug notice. Alto build tools will remove any calls to
     `Alto.debug()` when doing a production build.

     ```javascript
     Alto.debug("I'm a debug notice!");
     ```

     @method debug
     @param {String} message A debug message to display.
     */
    Alto.debug = function(message) {
        Alto.Logger.debug("DEBUG: "+message);
    };

    /**
     Display a deprecation warning with the provided message and a stack trace
     (Chrome and Firefox only). Alto build tools will remove any calls to
     `Alto.deprecate()` when doing a production build.
     ...
     will be displayed.
     */
    Alto.deprecate = function(message, test) {
        if (test) { return; }

        if (Alto.ENV.RAISE_ON_DEPRECATION) { throw new Alto.Error(message); }
        ...
        if (Alto.LOG_STACKTRACE_ON_DEPRECATION && error.stack) {
            var stack, stackStr = '';
            if (error['arguments']) {
                // Chrome
        ...
        Alto.Logger.warn("DEPRECATION: "+message);
    };
    ...
    /**
     ...
     Alto build tools will not remove calls to `Alto.deprecateFunc()`, though
     no warnings will be shown in production.

     ```javascript
     Alto.oldMethod = Alto.deprecateFunc("Please use the new, updated method", Alto.newMethod);
     ```
     ...
     */
    Alto.deprecateFunc = function(message, func) {
        return function() {
            Alto.deprecate(message);
    ...
    /**
     Run a function meant for debugging. Alto build tools will remove any calls to
     `Alto.runInDebug()` when doing a production build.

     ```javascript
     Alto.runInDebug( function() {
    Alto.Handlebars.EachView.reopen({
     ...
     */
    Alto.runInDebug = function(func) {
        func()
    };

// Inform the developer about the Alto Inspector if not installed.
    if (!Alto.testing) {
        ...
        if (typeof window !== 'undefined' && (isFirefox || isChrome) && window.addEventListener) {
            window.addEventListener("load", function() {
                if (document.documentElement && document.documentElement.dataset && !document.documentElement.dataset.emberExtension) {
                    ...
                    Alto.debug('For more advanced debugging, install the Alto Inspector from ' + downloadURL);
                }
            }, false);
        }
    }
...
(function() {
    ...
    (function() {
        /*globals Alto:true ENV EmberENV MetamorphENV:true */
...
// Default imports, exports and lookup to the global object;
        var imports = Alto.imports = Alto.imports || this;
        var exports = Alto.exports = Alto.exports || this;
        var lookup  = Alto.lookup  = Alto.lookup  || this;

// aliases needed to keep minifiers from removing the global context
        exports.Alto = exports.Alto = Alto = Alto;
...
// Make sure these are set whether Alto was already defined or not
        ...
        /**
         @property VERSION
         @type String
         @default '1.5.0-beta.5+pre.7af8beb0'
         @static
         */
        Alto.VERSION = '0.0.1';
        ...
        /**
         ...
         @static
         */
        Alto.RUNTIME_VERSION = '1.5.0-beta.5+pre.7af8beb0';
...
//
        ...
        /**
         Determines whether Alto should enhances some built-in object prototypes to
        ...
        /**
         Determines whether Alto should add ECMAScript 5 shims to older browsers.
...
// Stub out the methods defined by the ember-debug package in case it's not loaded
        ...
        /**
         ...
         @return {Object}
         */
        Alto.merge = function(original, updates) {
        ...
        /**
         ...
         @return {Boolean}
         */
        Alto.isNone = function(obj) {
            return obj === null || obj === undefined;
        };
        Alto.none = Alto.deprecateFunc("Alto.none is deprecated. Please use Alto.isNone instead.", Alto.isNone);

        /**
         Verifies that a value is `null` or an empty string, empty array,
         or empty function.

         Constrains the rules on `Alto.isNone` by returning false for empty
         ...
         @return {Boolean}
         */
        Alto.isEmpty = function(obj) {
            return Alto.isNone(obj) || (obj.length === 0 && typeof obj !== 'function') || (typeof obj === 'object' && Alto.get(obj, 'length') === 0);
        };
        Alto.empty = Alto.deprecateFunc("Alto.empty is deprecated. Please use Alto.isEmpty instead.", Alto.isEmpty);
        ...
        /**
         ...
         @return {Boolean}
         */
        Alto.isBlank = function(obj) {
            return Alto.isEmpty(obj) || (typeof obj === 'string' && obj.match(/\S/) === null);
        };
...
// Object.create different than we would prefer
        if (!Alto.create || Alto.ENV.STUB_OBJECT_CREATE) {
            var K = function() {};
...
// Catch IE8 where Object.defineProperty exists but only works on DOM elements
        ...
        if (Alto.ENV.MANDATORY_SETTER && !platform.hasPropertyAccessors) {
            Alto.ENV.MANDATORY_SETTER = false;
        }
...
// From: https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/array/foreach
    ...
    (function() {
        ...
        Alto.Error.prototype = Alto.create(Error.prototype);
...
//
    ...
    (function() {
        ...
        /**
         ...
         @private
         */
        Alto.GUID_PREFIX = 'alto-';


        var o_defineProperty = Alto.platform.defineProperty,
            o_create = Alto.create,
        // Used for guid generation...
            GUID_KEY = '__alto'+ (+ new Date()),
            numberCache  = [],
            stringCache  = {},
            uuid = 0;

        var MANDATORY_SETTER = Alto.ENV.MANDATORY_SETTER;
         ...
         */
        Alto.generateGuid = function generateGuid(obj, prefix) {
            if (!prefix) prefix = Alto.GUID_PREFIX;
            var ret = (prefix + (uuid++));
            if (obj) {
         ...
         */
        Alto.guidFor = function guidFor(obj) {
...
//
        ...
        var isDefinePropertySimulated = Alto.platform.defineProperty.isSimulated;
...
// Placeholder for non-writable metas.
         ...
         */
        Alto.metaPath = function metaPath(obj, path, writable) {
         ...
         */
        Alto.tryInvoke = function(obj, methodName, args) {
...
// https://github.com/emberjs/ember.js/pull/1617
    ...
    someResource.unlock();
        ...
        if (needsFinallyFix) {
            Alto.tryFinally = function(tryable, finalizer, binding) {
    ...
    for (i=0, l=listeners.length; i<l; i++) {
      ...
      listener.after(name, time(), payload, beforeValues[i]);
         ...
         */
        if (needsFinallyFix) {
            Alto.tryCatchFinally = function(tryable, catchable, finalizer, binding) {
                var result, finalResult, finalError;
...
//
        ...
        Alto.ArrayPolyfills.forEach.call(t, function(name) {
         ...
         */
        Alto.typeOf = function(item) {
            var ret;

            ret = (item === null || item === undefined) ? String(item) : TYPE_MAP[toString.call(item)] || 'object';

            if (ret === 'function') {
                if (Alto.Object && Alto.Object.detect(item)) ret = 'class';
            } else if (ret === 'object') {
                if (item instanceof Error) ret = 'error';
                else if (Alto.Object && item instanceof Alto.Object) ret = 'instance';
         ...
         */
        Alto.inspect = function(obj) {
            var type = Alto.typeOf(obj);
            if (type === 'array') {
                return '[' + obj + ']';
            }
            if (type !== 'object') {
            ...
            for(var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    ...
                    if (Alto.typeOf(v) === 'function') { v = "function() { ... }"; }
                    ret.push(key + ": " + v);
                }
            }
            return "{" + ret.join(", ") + "}";
...
// Alto.tryCatchFinally
    ...
    // rendering logic
         ...
         */
        Alto.Instrumentation.instrument = function(name, payload, callback, binding) {
            var listeners = cache[name], timeName, ret;

            if (Alto.STRUCTURED_PROFILE) {
            ...
            if (listeners.length === 0) {
                ...
                if (Alto.STRUCTURED_PROFILE) { console.timeEnd(timeName); }
                return ret;
            }
            ...
            return Alto.tryCatchFinally(tryable, catchable, finalizer);
        };
        ...
        /**
         ...
         @return {Subscriber}
         */
        Alto.Instrumentation.subscribe = function(pattern, object) {
         ...
         */
        Alto.Instrumentation.unsubscribe = function(subscriber) {
            var index;
        ...
        /**
         ...
         @namespace Alto.Instrumentation
         */
        Alto.Instrumentation.reset = function() {
            subscribers = [];
            cache = {};
        };

        Alto.instrument = Alto.Instrumentation.instrument;
        Alto.subscribe = Alto.Instrumentation.subscribe;
    })();
    ...
    (function() {
        ...
        map     = Array.prototype.map     || Alto.ArrayPolyfills.map;
        forEach = Array.prototype.forEach || Alto.ArrayPolyfills.forEach;
        indexOf = Array.prototype.indexOf || Alto.ArrayPolyfills.indexOf;
        filter = Array.prototype.filter || Alto.ArrayPolyfills.filter;
         ...
         * @static
         * */
        var utils = Alto.EnumerableUtils = {
            /**
    ...
    (function() {
        ...
        var META_KEY = Alto.META_KEY, get;
...
// Currently used only by Alto Data tests
         ...
         */
        var normalizeTuple = Alto.normalizeTuple = function(target, path) {
            ...
            if (!path || path.length===0) throw new Alto.Error('Path cannot be empty');

            return [ target, path ];
        };

        var getPath = Alto._getPath = function(root, path) {
            var hasThis, parts, tuple, idx, len;

            // If there is no root and path is a key name, return that
            // property from the global object.
            // E.g. get('Alto') -> Alto
            if (root === null && path.indexOf('.') === -1) { return get(Alto.lookup, path); }
        ...
        Alto.getWithDefault = function(root, key, defaultValue) {
    ...
    (function() {
        ...
        var o_create = Alto.create,
            metaFor = Alto.meta,
            META_KEY = Alto.META_KEY,
            a_slice = [].slice,
        /* listener flags */
            ONCE = 1, SUSPENDED = 2;
        ...
        function suspendListener(obj, eventName, target, method, callback) {
            ...
            return Alto.tryFinally(tryable, finalizer);
        }
    ...
    logCompleted: Alto.on('completed', function(){
      console.log('Job completed!');
        ...
        Alto.listenersDiff = actionsDiff;
        Alto.listenersUnion = actionsUnion;
    ...
    (function() {
         ...
         */
        var ObserverSet = Alto._ObserverSet = function() {
            this.clear();
        };
    ...
    (function() {
        var META_KEY = Alto.META_KEY,
            ...
            tryFinally = Alto.tryFinally,
            sendEvent = Alto.sendEvent,
            listenersUnion = Alto.listenersUnion,
            listenersDiff = Alto.listenersDiff,
            ObserverSet = Alto._ObserverSet,
...
// called whenever a property has just changed to update dependent keys
        ...
        Alto.overrideChains = function(obj, keyName, m) {
    ...
    obj2.set('bar', baz);
         ...
         @param [binding]
         */
        Alto.changeProperties = function(cb, binding) {
...
// propertyWillChange, propertyDidChange
        ...
        var META_KEY = Alto.META_KEY,
            MANDATORY_SETTER = Alto.ENV.MANDATORY_SETTER,
            IS_GLOBAL = /^([A-Z$]|([0-9][A-Z$]))/,
            getPath = Alto._getPath;
                    ...
                    if (value !== currentValue) {
                        ...
                        if (MANDATORY_SETTER) {
                            if ((currentValue === undefined && !(keyName in obj)) || !obj.propertyIsEnumerable(keyName)) {
                                Alto.defineProperty(obj, keyName, null, value); // setup mandatory setter
                            } else {
                        ...
                        Alto.propertyDidChange(obj, keyName);
                    }
                } else {
                    obj[keyName] = value;
                }
...
// Currently used only by Alto Data tests
        ...
        function setPath(root, path, value, tolerant) {
            ...
            if (!keyName || keyName.length === 0) {
                throw new Alto.Error('Property set failed: You passed an empty path');
            }

            if (!root) {
                if (tolerant) { return; }
                else { throw new Alto.Error('Property set failed: object in path "'+path+'" could not be found or was destroyed.'); }
            }
        ...
        /**
         ...
         @param {Object} value The value to set
         */
        Alto.trySet = function(root, path, value) {
    ...
    (function() {
        ...
        var set = Alto.set,
            guidFor = Alto.guidFor,
            indexOf = Alto.ArrayPolyfills.indexOf;
        ...
        /**
         ...
         @private
         */
        var OrderedSet = Alto.OrderedSet = function() {
            this.clear();
        };
        ...
        /**
         ...
         @constructor
         */
        var Map = Alto.Map = function() {
            this.keys = Alto.OrderedSet.create();
            this.values = {};
        };
         ...
         */
        var MapWithDefault = Alto.MapWithDefault = function(options) {
            Map.call(this);
        ...
        MapWithDefault.prototype = Alto.create(Map.prototype);
        ...
        function assertPolyfill(test, message) {
            if (!test) {
                try {
                    ...
                    throw new Alto.Error("assertion failed: " + message);
                } catch(error) {
                    setTimeout(function() {
                        throw error;
                    }, 0);
    ...
    (function() {
        ...
        var META_KEY = Alto.META_KEY,
            metaFor = Alto.meta,
            objectDefineProperty = Alto.platform.defineProperty;
...
//
        ...
        /**
         ...
         @constructor
         */
        Alto.Descriptor = function() {};

// ..........................................................
// DEFINING PROPERTIES API
//

        var MANDATORY_SETTER_FUNCTION = Alto.MANDATORY_SETTER_FUNCTION = function(value) {
            Alto.assert("You must use Alto.set() to access this property (of " + this + ")", false);
        };

        var DEFAULT_GETTER_FUNCTION = Alto.DEFAULT_GETTER_FUNCTION = function(name) {
            return function() {
    ...
    return this.firstName+' '+this.lastName;
         ...
         */
        Alto.defineProperty = function(obj, keyName, desc, data, meta) {
            ...
            if (existingDesc instanceof Alto.Descriptor) {
            ...
            if (desc instanceof Alto.Descriptor) {
                ...
                if (Alto.FEATURES.isEnabled('composable-computed-properties')) {
            ...
            if (watching) { Alto.overrideChains(obj, keyName, meta); }
        ...
        if (Alto.FEATURES.isEnabled('composable-computed-properties')) {
            var addImplicitCPs = function defineImplicitCPs(obj, implicitCPs, meta) {
                ...
                for (var i=0; i<length; ++i) {
                    ...
                    Alto.defineProperty(obj, key, cp, undefined, meta);
    ...
    (function() {
        ...
        /**
         ...
         @return {Hash}
         */
        Alto.getProperties = function(obj) {
            var ret = {},
                propertyNames = arguments,
                i = 1;

            if (arguments.length === 2 && Alto.typeOf(arguments[1]) === 'array') {
                i = 0;
    ...
    (function() {
        var changeProperties = Alto.changeProperties,
            set = Alto.set;
    ...
    age: "21"
         ...
         @return self
         */
        Alto.setProperties = function(self, hash) {
    ...
    (function() {
        var metaFor = Alto.meta, // utils.js
            typeOf = Alto.typeOf, // utils.js
            MANDATORY_SETTER = Alto.ENV.MANDATORY_SETTER,
            o_defineProperty = Alto.platform.defineProperty;

        Alto.watchKey = function(obj, keyName, meta) {
            ...
            if (!watching[keyName]) {
                ...
                if (MANDATORY_SETTER && keyName in obj) {
                    ...
                    o_defineProperty(obj, keyName, {
                        ...
                        set: Alto.MANDATORY_SETTER_FUNCTION,
                        get: Alto.DEFAULT_GETTER_FUNCTION(keyName)
                    });
                }
            } else {
        ...
        Alto.unwatchKey = function(obj, keyName, meta) {
            var m = meta || metaFor(obj), watching = m.watching;
    ...
    (function() {
        var metaFor = Alto.meta, // utils.js
            get = Alto.get, // property_get.js
            normalizeTuple = Alto.normalizeTuple, // property_get.js
            forEach = Alto.ArrayPolyfills.forEach, // array.js
            warn = Alto.warn,
            watchKey = Alto.watchKey,
            unwatchKey = Alto.unwatchKey,
            FIRST_KEY = /^([^\.\*]+)/,
            META_KEY = Alto.META_KEY;
...
// again.
        Alto.flushPendingChains = function() {
            ...
            warn('Watching an undefined global, Alto expects watched globals to be setup by the time the run loop is flushed, check for typos', pendingQueue.length === 0);
        };
        ...
        var removeChainWatcher = Alto.removeChainWatcher = function(obj, keyName, node) {
            if (!obj || 'object' !== typeof obj) { return; } // nothing to do
...
// pass null for parent and key and object for value.
        var ChainNode = Alto._ChainNode = function(parent, key, value) {
            this._parent = parent;
            this._key    = key;
...
// path
        ...
        Alto.finishChains = function(obj) {
    ...
    (function() {
        ...
        var forEach = Alto.EnumerableUtils.forEach,
            BRACE_EXPANSION = /^((?:[^\.]*\.)*)\{(.*)\}$/;
         ...
         */
        Alto.expandProperties = function (pattern, callback) {
    ...
    (function() {
        var metaFor = Alto.meta, // utils.js
            typeOf = Alto.typeOf, // utils.js
            ChainNode = Alto._ChainNode; // chains.js
...
// the current object.
        ...
        Alto.watchPath = function(obj, keyPath, meta) {
            // can't watch length on Array - it is special...
        ...
        Alto.unwatchPath = function(obj, keyPath, meta) {
            var m = meta || metaFor(obj), watching = m.watching;
    ...
    (function() {
        ...
        var metaFor = Alto.meta, // utils.js
            GUID_KEY = Alto.GUID_KEY, // utils.js
            META_KEY = Alto.META_KEY, // utils.js
            removeChainWatcher = Alto.removeChainWatcher,
            watchKey = Alto.watchKey, // watch_key.js
            unwatchKey = Alto.unwatchKey,
            watchPath = Alto.watchPath, // watch_path.js
            unwatchPath = Alto.unwatchPath,
            typeOf = Alto.typeOf, // utils.js
            generateGuid = Alto.generateGuid,
            IS_PATH = /[\.\*]/;
...
// returns true if the passed path is just a keyName
         ...
         */
        Alto.watch = function(obj, _keyPath, m) {
            // can't watch length on Array - it is special...
        ...
        Alto.isWatching = function isWatching(obj, key) {
            var meta = obj[META_KEY];
            return (meta && meta.watching[key]) > 0;
        };

        Alto.watch.flushPending = Alto.flushPendingChains;

        Alto.unwatch = function(obj, _keyPath, m) {
            // can't watch length on Array - it is special...
            if (_keyPath === 'length' && typeOf(obj) === 'array') { return; }
    ...
    (function() {
        ...
        Alto.warn("The CP_DEFAULT_CACHEABLE flag has been removed and computed properties are always cached by default. Use `volatile` if you don't want caching.", Alto.ENV.CP_DEFAULT_CACHEABLE !== false);
        ...
        var get = Alto.get,
            ...
            META_KEY = Alto.META_KEY,
            watch = Alto.watch,
            unwatch = Alto.unwatch;

        var expandProperties = Alto.expandProperties;
...
//
      ...
      // setter
        ...
        if (Alto.FEATURES.isEnabled('composable-computed-properties')) {
            ...
            ComputedPropertyPrototype.toString = function() {
                ...
                return Alto.Descriptor.prototype.toString.apply(this, arguments);
            };
        }
        ...
        /**
         ...
         ```javascript
         MyApp.outsideService = Alto.Object.extend({
    value: function() {
      return OutsideService.getValue();
        ...
        /**
         ...
         ```javascript
         MyApp.Person = Alto.Object.extend({
    guid: function() {
    ...
    fullName: Alto.computed(function() {
    ...
    lastName: "Dale"
        ...
        ComputedPropertyPrototype.set = function(obj, keyName, value) {
            ...
            if (this._readOnly) {
                throw new Alto.Error('Cannot set read-only property "' + keyName + '" on object: ' + Alto.inspect(obj));
            }
            ...
            try {
                ...
                if (funcArgLength === 3) {
                    ...
                    Alto.defineProperty(obj, keyName, null, cachedValue);
                    Alto.set(obj, keyName, value);
                    return;
                }

                if (hadCachedValue && cachedValue === ret) { return; }

                if (watched) { Alto.propertyWillChange(obj, keyName); }
        ...
        Alto.computed = function(func) {
            ...
            if (typeof func !== "function") {
                throw new Alto.Error("Computed Property declared without a property function");
            }
         ...
         */
        Alto.cacheFor = function cacheFor(obj, key) {
            var meta = obj[META_KEY],
        ...
        if (Alto.FEATURES.isEnabled('composable-computed-properties')) {
            var guidFor = Alto.guidFor,
                map = Alto.EnumerableUtils.map,
                filter = Alto.EnumerableUtils.filter,
                typeOf = Alto.typeOf;
            ...
            var normalizeDependentKey = function (key) {
                if (key instanceof Alto.ComputedProperty) {
            ...
            var selectDependentCPs = function (keys) {
                return filter(keys, function (key) {
                    return key instanceof Alto.ComputedProperty;
                });
            };
            ...
            // composition
            Alto.computed.normalizeDependentKey = normalizeDependentKey;
            Alto.computed.normalizeDependentKeys = normalizeDependentKeys;

            registerComputed = function (name, macro) {
                Alto.computed[name] = function(dependentKey) {
                    var args = normalizeDependentKeys(a_slice.call(arguments));
                    return Alto.computed(dependentKey, function() {
        ...
        if (Alto.FEATURES.isEnabled('composable-computed-properties')) {
            registerComputedWithProperties = function(name, macro) {
                Alto.computed[name] = function() {
                    var args = a_slice.call(arguments);
                    var properties = normalizeDependentKeys(args);

                    var computed = Alto.computed(function() {
        ...
        if (Alto.FEATURES.isEnabled('composable-computed-properties')) {
            Alto.computed.literal = function (value) {
                return Alto.computed(function () {
                    return value;
                });
            };
        }
        ...
        /**
         ...
         Note: When using `Alto.computed.empty` to watch an array make sure to
      ...
      done: Alto.computed.empty('todos.[]') // detect array changes
    });
        ...
        registerComputed('empty', function(dependentKey) {
            return Alto.isEmpty(get(this, dependentKey));
        });
        ...
        /**
         ...
         Note: When using `Alto.computed.notEmpty` to watch an array make sure to
         use the `array.[]` syntax so the computed can subscribe to transitions
         from empty to non-empty states.
    ...
    hasStuff: Alto.computed.notEmpty('backpack.[]')
  });
    ...
    isHungry: Alto.computed.none('food')
        ...
        registerComputed('none', function(dependentKey) {
            return Alto.isNone(get(this, dependentKey));
        });
    ...
    hasBananas: Alto.computed.bool('numBananas')
        ...
        /**
         ...
         ```javascript
         var User = Alto.Object.extend({
    hasValidEmail: Alto.computed.match('email', /^.+@.+\..+$/)
         });
         var user = User.create({loggedIn: false});
    ...
    nomen: Alto.computed.alias('name')
         ...
         */
        Alto.computed.alias = function(dependentKey) {
            return Alto.computed(dependentKey, function(key, value) {
                if (arguments.length > 1) {
    ...
    lastName:  'Zeenny'
         ...
         */
        Alto.computed.oneWay = function(dependentKey) {
            return Alto.computed(dependentKey, function() {
                return get(this, dependentKey);
            });
        };
    ...
    lastName:  'Zeenny'
         ...
         one way computed property to the original value for property.
         */
        Alto.computed.readOnly = function(dependentKey) {
            return Alto.computed(dependentKey, function() {
                return get(this, dependentKey);
            }).readOnly();
        };
    ...
    wishList: Alto.computed.defaultTo('favoriteFood')
         ...
         */
        Alto.computed.defaultTo = function(defaultPath) {
            return Alto.computed(function(key, newValue, cachedValue) {
                if (arguments.length === 1) {
...
// Alto.tryFinally
         ...
         */
        Alto.addObserver = function(obj, _path, target, method) {
            Alto.addListener(obj, changeEvent(_path), target, method);
            Alto.watch(obj, _path);
        ...
        /**
         ...
         @param {Function|String} [method]
         */
        Alto.removeObserver = function(obj, _path, target, method) {
            Alto.unwatch(obj, _path);
            Alto.removeListener(obj, changeEvent(_path), target, method);
        ...
        /**
         ...
         @param {Function|String} [method]
         */
        Alto.addBeforeObserver = function(obj, _path, target, method) {
            Alto.addListener(obj, beforeEvent(_path), target, method);
            Alto.watch(obj, _path);
...
// while it is setting the observed path.
        Alto._suspendBeforeObserver = function(obj, path, target, method, callback) {
            return Alto._suspendListener(obj, beforeEvent(path), target, method, callback);
        };

        Alto._suspendObserver = function(obj, path, target, method, callback) {
            return Alto._suspendListener(obj, changeEvent(path), target, method, callback);
        };

        var map = Alto.ArrayPolyfills.map;

        Alto._suspendBeforeObservers = function(obj, paths, target, method, callback) {
            var events = map.call(paths, beforeEvent);
            return Alto._suspendListeners(obj, events, target, method, callback);
        };

        Alto._suspendObservers = function(obj, paths, target, method, callback) {
        ...
        /**
         ...
         @param {Function|String} [method]
         */
        Alto.removeBeforeObserver = function(obj, _path, target, method) {
            Alto.unwatch(obj, _path);
            Alto.removeListener(obj, beforeEvent(_path), target, method);
        ...
        define("backburner/queue",
            ...
            function(__exports__) {
                ...
                Queue.prototype = {
                    ...
                    // TODO: remove me, only being used for Alto.run.sync
                    flush: function() {
                        var queue = this._queue,
...
//
    ...
    Alto.run.join(function() {
      ...
      // the existing run-loops action queue.
         ...
         */
        Alto.run.join = function(target, method /* args */) {
            if (!Alto.run.currentRunLoop) {
                return Alto.run.apply(Alto.run, arguments);
            }

            var args = slice.call(arguments);
            args.unshift('actions');
            Alto.run.schedule.apply(Alto.run, args);
        };

        /**
         Provides a useful utility for when integrating with non-Alto libraries
         that provide asynchronous callbacks.

         Alto utilizes a run-loop to batch and coalesce changes. This works by
         marking the start and end of Alto-related Javascript execution.

         When using events such as a View's click handler, Alto wraps the event
         handler in a run-loop, but when integrating with non-Alto libraries this
         can be tedious.

         For example, the following is rather verbose but is the correct way to combine
         third-party events and Alto code.
    ...
    Alto.run(function(){
      that.handleResize();
         ...
         ```javascript
         jQuery(window).on('resize', Alto.run.bind(this, this.handleResize));
         ```
         ...
         when called within an existing loop, no return value is possible.
         */
        Alto.run.bind = function(target, method /* args*/) {
            var args = slice.call(arguments);
            return function() {
                return Alto.run.join.apply(Alto.run, args.concat(slice.call(arguments)));
            };
        };

        Alto.run.backburner = backburner;

        var run = Alto.run;

        Alto.run.currentRunLoop = null;

        Alto.run.queues = backburner.queueNames;

        /**
         Begins a new RunLoop. Any deferred actions invoked after the begin will
         be buffered until you invoke a matching call to `Alto.run.end()`. This is
         a lower-level way to use a RunLoop instead of using `Alto.run()`.
        ...
        /**
         ...
         `Alto.run.begin()` to flush any deferred actions. This is a lower-level way
         to use a RunLoop instead of using `Alto.run()`.
        ...
        /**
         ...
         the `Alto.run.queues` property.

         ```javascript
         Alto.run.schedule('sync', this, function() {
    // this will be executed in the first RunLoop queue, when bindings are synced
    console.log("scheduled on sync queue");
  });

         Alto.run.schedule('actions', this, function() {
...
// Used by global test teardown
        Alto.run.hasScheduledTimers = function() {
            return backburner.hasTimers();
        };

// Used by global test teardown
        Alto.run.cancelTimers = function () {
        ...
        /**
         ...
         ```javascript
         Alto.run.later(myContext, function() {
    // code here will execute within a RunLoop in about 500ms with this == myContext
        ...
        /**
         ...
         target at the time the method is invoked.
         @param {Object} [args*] Optional arguments to pass to the timeout.
         @return {Object} Timer information for use in cancelling, see `Alto.run.cancel`.
         */
    ...
    Alto.run.scheduleOnce('afterRender', myContext, sayHi);
    Alto.run.scheduleOnce('afterRender', myContext, sayHi);
    // sayHi will only be executed once, in the afterRender queue of the RunLoop
  });
         ```

         Also note that passing an anonymous function to `Alto.run.scheduleOnce` will
         not prevent additional calls with an identical anonymous function from
         scheduling the items multiple times, e.g.:

         ```javascript
         function scheduleIt() {
    Alto.run.scheduleOnce('actions', myContext, function() { console.log("Closure"); });
  }
         scheduleIt();
         scheduleIt();
         // "Closure" will print twice, even though we're using `Alto.run.scheduleOnce`,
         // because the function we pass to it is anonymous and won't match the
         // previously scheduled operation.
         ```

         Available queues, and their order, can be found at `Alto.run.queues`
         ...
         @return {Object} Timer information for use in cancelling, see `Alto.run.cancel`.
         */
        Alto.run.scheduleOnce = function(queue, target, method) {
            checkAutoRun();
        ...
        /**
         ...
         `Alto.run.later` with a wait time of 1ms.

         ```javascript
         Alto.run.next(myContext, function() {
    // code to be executed in the next run loop,
    // which will be scheduled after the current one
  });
         ```

         Multiple operations scheduled with `Alto.run.next` will coalesce
         into the same later run loop, along with any other operations
         scheduled by `Alto.run.later` that expire right around the same
         time that `Alto.run.next` operations will fire.

         Note that there are often alternatives to using `Alto.run.next`.
    ...
    didInsertElement: function() {
      Alto.run.scheduleOnce('afterRender', this, 'processChildElements');
    },
    processChildElements: function() {
      ...
      // before the child elements have been added to the DOM.
         ...
         One benefit of the above approach compared to using `Alto.run.next` is
         that you will be able to perform DOM/CSS operations before unprocessed
         elements are rendered to the screen, which may prevent flickering or
         other artifacts caused by delaying processing until after rendering.

         The other major benefit to the above approach is that `Alto.run.next`
         introduces an element of non-determinism, which can make things much
         harder to test, due to its reliance on `setTimeout`; it's much harder
         to guarantee the order of scheduled operations when they are scheduled
         outside of the current run loop, i.e. with `Alto.run.next`.
        ...
        /**
         ...
         `Alto.run.once()`, `Alto.run.next()`, `Alto.run.debounce()`, or
         `Alto.run.throttle()`.

         ```javascript
         var runNext = Alto.run.next(myContext, function() {
    // will not be executed
  });
         Alto.run.cancel(runNext);

         var runLater = Alto.run.later(myContext, function() {
    // will not be executed
  }, 500);
         Alto.run.cancel(runLater);

         var runOnce = Alto.run.once(myContext, function() {
    // will not be executed
  });
         Alto.run.cancel(runOnce);

         var throttle = Alto.run.throttle(myContext, function() {
    // will not be executed
  }, 1, false);
         Alto.run.cancel(throttle);

         var debounce = Alto.run.debounce(myContext, function() {
    // will not be executed
  }, 1);
         Alto.run.cancel(debounce);

         var debounceImmediate = Alto.run.debounce(myContext, function() {
    // will be executed since we passed in true (immediate)
  }, 100, true);
         // the 100ms delay until this method can be called again will be cancelled
         Alto.run.cancel(debounceImmediate);
         ```
        ...
        /**
         ...
         Alto.run.debounce(myContext, myFunc, 150);
         ...
         Alto.run.debounce(myContext, myFunc, 150, true);
         ...
         @return {Array} Timer information for use in cancelling, see `Alto.run.cancel`.
         */
        Alto.run.debounce = function() {
        ...
        /**
         ...
         Alto.run.throttle(myContext, myFunc, 150);
         // myFunc is invoked with context myContext
...
// Make sure it's not an autorun during testing
        function checkAutoRun() {
            if (!Alto.run.currentRunLoop) {
                Alto.assert("You have turned on testing mode, which disabled the run-loop's autorun. You will need to wrap any code with asynchronous side-effects in an Alto.run", !Alto.testing);
            }
        }
...
//
        ...
        /**
         ...
         @return Boolean
         */
        var isGlobalPath = Alto.isGlobalPath = function(path) {
...
//
        ...
        var Binding = function(toPath, fromPath) {
            ...
            this._from = fromPath;
            this._to   = toPath;
            this._directionMap = Alto.Map.create();
        };
        ...
        Binding.prototype = {
            ...
            connect: function(obj) {
                ...
                Alto.trySet(obj, toPath, getWithGlobals(obj, fromPath));

                // add an observer on the object to be notified when the binding should be updated
                Alto.addObserver(obj, fromPath, this, this.fromDidChange);

                // if the binding is a two-way binding, also set up an observer on the target
                if (!this._oneWay) { Alto.addObserver(obj, toPath, this, this.toDidChange); }
            ...
            disconnect: function(obj) {
                ...
                if (twoWay) { Alto.removeObserver(obj, this._to, this, this.toDidChange); }
            ...
            _sync: function(obj) {
                ...
                if (direction === 'fwd') {
                    ...
                    if (this._oneWay) {
                        Alto.trySet(obj, toPath, fromValue);
                    } else {
                        Alto._suspendObserver(obj, toPath, this, this.toDidChange, function () {
                            Alto.trySet(obj, toPath, fromValue);
                        });
                    }
                    ...
                    Alto._suspendObserver(obj, fromPath, this, this.fromDidChange, function () {
                        Alto.trySet(Alto.isGlobalPath(fromPath) ? Alto.lookup : obj, fromPath, toValue);
                    });
                }
            }
    ...
    (function() {
        ...
        var Mixin, REQUIRED, Alias,
            a_map = Alto.ArrayPolyfills.map,
            a_indexOf = Alto.ArrayPolyfills.indexOf,
            a_forEach = Alto.ArrayPolyfills.forEach,
            a_slice = [].slice,
            o_create = Alto.create,
            defineProperty = Alto.defineProperty,
            guidFor = Alto.guidFor,
            metaFor = Alto.meta,
            META_KEY = Alto.META_KEY;
        ...
        function giveDescriptorSuper(meta, key, property, values, descs) {
            ...
            if (!superProperty || !(superProperty instanceof Alto.ComputedProperty)) {
                return property;
            }
            ...
            property.func = Alto.wrap(property.func, superProperty.func);
        ...
        function giveMethodSuper(obj, key, method, values, descs) {
            ...
            return Alto.wrap(method, superMethod);
        }
        ...
        function applyConcatenatedProperties(obj, key, value, values) {
            ...
            if (baseValue) {
                if ('function' === typeof baseValue.concat) {
                    ...
                    return Alto.makeArray(baseValue).concat(value);
                }
            } else {
                return Alto.makeArray(value);
            }
        }
        ...
        function applyMergedProperties(obj, key, value, values) {
            ...
            var newBase = Alto.merge({}, baseValue),
                hasFunction = false;
        ...
        var IS_BINDING = Alto.IS_BINDING = /^.+Binding$/;
            ...
            if (bindings) {
                for (key in bindings) {
                    ...
                    if (binding) {
                        ...
                        if (binding instanceof Alto.Binding) {
                            binding = binding.copy(); // copy prototypes' instance
                            binding.to(to);
                        } else { // binding is string path
                            binding = new Alto.Binding(to, binding);
                        }
                        binding.connect(obj);
                        obj[key] = binding;
                    }
        ...
        function updateObserversAndListeners(obj, key, observerOrListener, pathsKey, updateMethod) {
            ...
            if (paths) {
                for (var i=0, l=paths.length; i<l; i++) {
                    Alto[updateMethod](obj, paths[i], null, key);
                }
            }
        }
    ...
    init: function() {
      ...
      this.set("filters", Alto.A());
         ...
         @namespace Alto
         */
        Alto.Mixin = function() { return initMixin(this, arguments); };
...
// TODO: Make Alto.mixin
    ...
    moniker: Alto.aliasMethod('name')
         ...
         */
        Alto.aliasMethod = function(methodName) {
...
//
    ...
    valueObserver: Alto.immediateObserver('value', function() {
      // Executes whenever the "value" property changes
         ...
         @return func
         */
        Alto.immediateObserver = function() {
            for (var i=0, l=arguments.length; i<l; i++) {
                var arg = arguments[i];
                Alto.assert("Immediate observers must observe internal properties only, not properties on other objects.", typeof arg !== "string" || arg.indexOf('.') === -1);
            }

            return Alto.observer.apply(this, arguments);
        };
...
// Provides a way to register library versions with ember.
        var forEach = Alto.EnumerableUtils.forEach,
            indexOf = Alto.EnumerableUtils.indexOf;

        Alto.libraries = function() {
            var libraries    = [];
            var coreLibIndex = 0;
    ...
    (function() {
        ...
        define("container",
            ...
            function(__dependency1__, __exports__) {
                 ...
                 */
                Alto.MODEL_FACTORY_INJECTIONS = false || !!Alto.ENV.MODEL_FACTORY_INJECTIONS;
    ...
    (function() {
        ...
        var indexOf = Alto.EnumerableUtils.indexOf;
         ...
         */
        Alto.compare = function compare(v, w) {
            if (v === w) { return 0; }

            var type1 = Alto.typeOf(v);
            var type2 = Alto.typeOf(w);

            var Comparable = Alto.Comparable;
            if (Comparable) {
            ...
            // If we haven't yet generated a reverse-mapping of Alto.ORDER_DEFINITION,
            // do so now.
            var mapping = Alto.ORDER_DEFINITION_MAPPING;
            if (!mapping) {
                var order = Alto.ORDER_DEFINITION;
                mapping = Alto.ORDER_DEFINITION_MAPPING = {};
                var idx, len;
                for (idx = 0, len = order.length; idx < len;  ++idx) {
                    mapping[order[idx]] = idx;
                }

                // We no longer need Alto.ORDER_DEFINITION.
                delete Alto.ORDER_DEFINITION;
            }
            ...
            switch (type1) {
                ...
                case 'instance':
                    if (Alto.Comparable && Alto.Comparable.detect(v)) {
        ...
        function _copy(obj, deep, seen, copies) {
            ...
            Alto.assert('Cannot clone an Alto.Object that does not implement Alto.Copyable', !(obj instanceof Alto.Object) || (Alto.Copyable && Alto.Copyable.detect(obj)));

            // IMPORTANT: this specific test will detect a native array only. Any other
            // object will need to implement Copyable.
            if (Alto.typeOf(obj) === 'array') {
                ret = obj.slice();
                if (deep) {
                    loc = ret.length;
                    while(--loc>=0) ret[loc] = _copy(ret[loc], deep, seen, copies);
                }
            } else if (Alto.Copyable && Alto.Copyable.detect(obj)) {
         ...
         */
        Alto.copy = function(obj, deep) {
            // fast paths
            if ('object' !== typeof obj || obj===null) return obj; // can't copy primitives
            if (Alto.Copyable && Alto.Copyable.detect(obj)) return obj.copy(deep);
        ...
        /**
         ...
         @return {Boolean}
         */
        Alto.isEqual = function(a, b) {
            if (a && 'function'===typeof a.isEqual) return a.isEqual(b);
            return a === b;
        };

// Used by Alto.compare
        Alto.ORDER_DEFINITION = Alto.ENV.ORDER_DEFINITION || [
            'undefined',
            'null',
            'boolean',
            'number',
         ...
         */
        Alto.keys = Object.keys;

        if (!Alto.keys || Alto.create.isSimulated) {
    ...
    (function() {
        ...
        Alto.String = {
            ...
            fmt: function(str, formats) {
                ...
                return str.replace(/%@([0-9]+)?/g, function(s, argIndex) {
                    ...
                    s = formats[argIndex];
                    return (s === null) ? '(null)' : (s === undefined) ? '' : Alto.inspect(s);
                }) ;
            },
      ...
      '_Hello %@ %@': 'Bonjour %@ %@'
            ...
            loc: function(str, formats) {
                ...
                return Alto.String.fmt(str, formats) ;
            },
      ...
      console.log(key);
            ...
            dasherize: function(str) {
                ...
                if (hit) {
                    ...
                    ret = Alto.String.decamelize(str).replace(STRING_DASHERIZE_REGEXP,'-');
                    cache[str] = ret;
                }
            ...
            classify: function(str) {
                ...
                for (var i=0, l=parts.length; i<l; i++) {
                    var camelized = Alto.String.camelize(parts[i]);
    ...
    (function() {
        ...
        var fmt = Alto.String.fmt,
            w   = Alto.String.w,
            loc = Alto.String.loc,
            camelize = Alto.String.camelize,
            decamelize = Alto.String.decamelize,
            dasherize = Alto.String.dasherize,
            underscore = Alto.String.underscore,
            capitalize = Alto.String.capitalize,
            classify = Alto.String.classify;
    ...
    (function() {
        ...
        var get = Alto.get,
            ...
            getProperties = Alto.getProperties;
    ...
    valueObserver: function() {
      // Executes whenever the "value" property changes
         ...
         @namespace Alto
         */
        Alto.Observable = Alto.Mixin.create({
      ...
      return this.get('firstName') + ' ' + this.get('lastName');
            ...
            setProperties: function(hash) {
                return Alto.setProperties(this, hash);
            },
            ...
            propertyDidChange: function(keyName) {
                Alto.propertyDidChange(this, keyName);
                return this;
            },
            ...
            addBeforeObserver: function(key, target, method) {
                Alto.addBeforeObserver(this, key, target, method);
            },
            ...
            /**
             ...
             @return {Alto.Object} self
             */
            addObserver: function(key, target, method) {
                Alto.addObserver(this, key, target, method);
            },
            ...
            /**
             ...
             @param {Object} target The target object to invoke
             @param {String|Function} method The method to invoke.
             @return {Alto.Observable} receiver
             */
            removeObserver: function(key, target, method) {
                Alto.removeObserver(this, key, target, method);
            },
            ...
            hasObserverFor: function(key) {
                return Alto.hasListeners(this, key+':change');
            },
            ...
            getWithDefault: function(keyName, defaultValue) {
                return Alto.getWithDefault(this, keyName, defaultValue);
            },
            ...
            incrementProperty: function(keyName, increment) {
                if (Alto.isNone(increment)) { increment = 1; }
            ...
            decrementProperty: function(keyName, decrement) {
                if (Alto.isNone(decrement)) { decrement = 1; }
            ...
            cacheFor: function(keyName) {
                return Alto.cacheFor(this, keyName);
            },

            // intended for debugging purposes
            observersForKey: function(keyName) {
                return Alto.observersFor(this, keyName);
            }
        });
...
// We only define this separately so that `Alto.Set` can depend on it.
        ...
        var set = Alto.set, get = Alto.get,
            o_create = Alto.create,
            o_defineProperty = Alto.platform.defineProperty,
            GUID_KEY = Alto.GUID_KEY,
            guidFor = Alto.guidFor,
            generateGuid = Alto.generateGuid,
            meta = Alto.meta,
            META_KEY = Alto.META_KEY,
            rewatch = Alto.rewatch,
            finishChains = Alto.finishChains,
            sendEvent = Alto.sendEvent,
            destroy = Alto.destroy,
            schedule = Alto.run.schedule,
            Mixin = Alto.Mixin,
            ...
            var Class = function() {
                ...
                if (initProperties) {
                    ...
                    for (var i = 0, l = props.length; i < l; i++) {
                        ...
                        Alto.assert("Alto.Object.create no longer supports mixing in other definitions, use createWithMixins instead.", !(properties instanceof Alto.Mixin));

                        if (typeof properties !== 'object' && properties !== undefined) {
                            throw new Alto.Error("Alto.Object.create only accepts objects.");
                        }

                        if (!properties) { continue; }

                        var keyNames = Alto.keys(properties);
                        ...
                        for (var j = 0, ll = keyNames.length; j < ll; j++) {
                            ...
                            Alto.assert("Alto.Object.create no longer supports defining computed properties. Define computed properties using extend() or reopen() before calling create().", !(value instanceof Alto.ComputedProperty));
                            ...
                            "controllers & routes).", !((keyName === 'actions') && Alto.ActionHandler.detect(this)));
                            ...
                            if (concatenatedProperties && indexOf(concatenatedProperties, keyName) >= 0) {
                                ...
                                if (baseValue) {
                                    if ('function' === typeof baseValue.concat) {
                                        ...
                                        value = Alto.makeArray(baseValue).concat(value);
                                    }
                                } else {
                                    value = Alto.makeArray(value);
                                }
                            }
                            ...
                            if (desc) {
                                ...
                                if (typeof this.setUnknownProperty === 'function' && !(keyName in this)) {
                                    ...
                                    Alto.defineProperty(this, keyName, null, value); // setup mandatory setter
                                } else {
                                    this[keyName] = value;
                                }
                            }
      ...
      classNames: 'baz'
            ...
            bind: function(to, from) {
                if (!(from instanceof Alto.Binding)) { from = Alto.Binding.from(from); }
      ...
      toStringExtension: function() {
        ...
        var ClassMixin = Mixin.create({
            ...
            ClassMixin: Alto.required(),

            PrototypeMixin: Alto.required(),
      ...
      name: "Yehuda Katz"
             ...
             You can also pass `Alto.Mixin` classes to add additional properties to the subclass.
      ...
      say: function(thing) {
        alert(this.get('name') + ' says: ' + thing);
             ...
             App.SingingMixin = Alto.Mixin.create({
      sing: function(thing){
      ...
      dance: function() {
        alert(this.get('name') + ' dances: tap tap tap tap ');
             ...
             @param {Alto.Mixin} [mixins]* One or more Alto.Mixin classes
      ...
      return App.Person.create({ id: personId });
            ...
            metaForProperty: function(key) {
                ...
                Alto.assert("metaForProperty() could not find a computed property with key '"+key+"'.", !!desc && desc instanceof Alto.ComputedProperty);
                return desc._meta || {};
            },
            ...
            eachComputedProperty: function(callback, binding) {
                ...
                for (var name in descs) {
                    ...
                    if (property instanceof Alto.ComputedProperty) {
    ...
    (function() {
         ...
         */
        Alto.Object = Alto.CoreObject.extend(Alto.Observable);
        Alto.Object.toString = function() { return "Alto.Object"; };
    ...
    (function() {
        ...
        var get = Alto.get, indexOf = Alto.ArrayPolyfills.indexOf;
    ...
    VERSION: '1.0.0'
         ...
         @extends Alto.Object
         */
        var Namespace = Alto.Namespace = Alto.Object.extend({
            isNamespace: true,

            init: function() {
                Alto.Namespace.NAMESPACES.push(this);
                Alto.Namespace.PROCESSED = false;
            },
            ...
            toString: function() {
                ...
                findNamespaces();
                return this[Alto.GUID_KEY+'_name'];
            },
            ...
            destroy: function() {
                var namespaces = Alto.Namespace.NAMESPACES;

                Alto.lookup[this.toString()] = undefined;
                delete Alto.Namespace.NAMESPACES_BY_ID[this.toString()];
        ...
        function findNamespaces() {
            var Namespace = Alto.Namespace, lookup = Alto.lookup, obj, isNamespace;
            ...
            for (var prop in lookup) {
                ...
                if (isNamespace) {
                    Alto.deprecate("Namespaces should not begin with lowercase.", /^[A-Z]/.test(prop));
                    obj[NAME_KEY] = prop;
                }
            }
        }

        var NAME_KEY = Alto.NAME_KEY = Alto.GUID_KEY + '_name';
        ...
        function processAllNamespaces() {
            var unprocessedNamespaces = !Namespace.PROCESSED,
                unprocessedMixins = Alto.anyUnprocessedMixins;
        ...
        Alto.Mixin.prototype.toString = classToString;
    ...
    (function() {
        ...
        var get = Alto.get,
            set = Alto.set,
            fmt = Alto.String.fmt,
            addBeforeObserver = Alto.addBeforeObserver,
            addObserver = Alto.addObserver,
            removeBeforeObserver = Alto.removeBeforeObserver,
            removeObserver = Alto.removeObserver,
            propertyWillChange = Alto.propertyWillChange,
            propertyDidChange = Alto.propertyDidChange,
            meta = Alto.meta,
            defineProperty = Alto.defineProperty;
    ...
    firstName: 'Tom', lastName: 'Dale'
         ...
         @extends Alto.Object
         */
        Alto.ObjectProxy = Alto.Object.extend({
            /**
             ...
             */
            content: null,
            _contentDidChange: Alto.observer('content', function() {
                Alto.assert("Can't set ObjectProxy's content to itself", this.get('content') !== this);
            }),

            isTruthy: Alto.computed.bool('content'),
            ...
            setUnknownProperty: function (key, value) {
                ...
                var content = get(this, 'content');
                Alto.assert(fmt("Cannot delegate set('%@', %@) to the 'content' property of object proxy %@: its 'content' is undefined.", [key, value, this]), content);
...
//
        ...
        var a_indexOf = Alto.EnumerableUtils.indexOf;
        ...
        /**
         ...
         @since Alto 0.9
         */
        Alto.Enumerable = Alto.Mixin.create({
             ...
             */
            nextObject: Alto.required(Function),
             ...
             */
            firstObject: Alto.computed(function() {
             ...
             */
            lastObject: Alto.computed(function() {
            ...
            mapProperty: Alto.aliasMethod('mapBy'),
             ...
             */
            filterProperty: Alto.aliasMethod('filterBy'),
             ...
             */
            rejectProperty: Alto.aliasMethod('rejectBy'),
             ...
             */
            findProperty: Alto.aliasMethod('findBy'),
            ...
            /**
             ...
             @return {Boolean}
             */
            everyBy: Alto.aliasMethod('isEvery'),
            ...
            /**
             ...
             @return {Boolean}
             */
            everyProperty: Alto.aliasMethod('isEvery'),
            ...
            /**
             ...
             @deprecated Use `isAny` instead
             */
            someProperty: Alto.aliasMethod('isAny'),
            ...
            /**
             ...
             For plain enumerables, this property is read only. `Alto.Array` overrides
             this method.
             ...
             @return this
             */
            '[]': Alto.computed(function(key, value) {
                return this;
            }),
            ...
            addEnumerableObserver: function(target, opts) {
                ...
                if (!hasObservers) Alto.propertyWillChange(this, 'hasEnumerableObservers');
                Alto.addListener(this, '@enumerable:before', target, willChange);
                Alto.addListener(this, '@enumerable:change', target, didChange);
                if (!hasObservers) Alto.propertyDidChange(this, 'hasEnumerableObservers');
                return this;
            },
            ...
            removeEnumerableObserver: function(target, opts) {
                ...
                var hasObservers = get(this, 'hasEnumerableObservers');
                if (hasObservers) Alto.propertyWillChange(this, 'hasEnumerableObservers');
                Alto.removeListener(this, '@enumerable:before', target, willChange);
                Alto.removeListener(this, '@enumerable:change', target, didChange);
                if (hasObservers) Alto.propertyDidChange(this, 'hasEnumerableObservers');
                return this;
            },
            ...
            /**
             ...
             @type Boolean
             */
            hasEnumerableObservers: Alto.computed(function() {
                return Alto.hasListeners(this, '@enumerable:change') || Alto.hasListeners(this, '@enumerable:before');
            }),
            ...
            enumerableContentWillChange: function(removing, adding) {
                ...
                Alto.propertyWillChange(this, '[]');
                if (hasDelta) Alto.propertyWillChange(this, 'length');
                Alto.sendEvent(this, '@enumerable:before', [this, removing, adding]);
            ...
            enumerableContentDidChange: function(removing, adding) {
                ...
                Alto.sendEvent(this, '@enumerable:change', [this, removing, adding]);
                if (hasDelta) Alto.propertyDidChange(this, 'length');
                Alto.propertyDidChange(this, '[]');
            ...
            sortBy: function() {
                ...
                return this.toArray().sort(function(a, b){
                    for(var i = 0; i < sortKeys.length; i++) {
                        ...
                        var compareValue = Alto.compare(propA, propB);
...
//
        ...
        var get = Alto.get, set = Alto.set, isNone = Alto.isNone, map = Alto.EnumerableUtils.map, cacheFor = Alto.cacheFor;
...
//
        /**
         ...
         @since Alto 0.9.0
         */
        Alto.Array = Alto.Mixin.create(Alto.Enumerable, {
            ...
            addArrayObserver: function(target, opts) {
                ...
                if (!hasObservers) Alto.propertyWillChange(this, 'hasArrayObservers');
                Alto.addListener(this, '@array:before', target, willChange);
                Alto.addListener(this, '@array:change', target, didChange);
                if (!hasObservers) Alto.propertyDidChange(this, 'hasArrayObservers');
                return this;
            },
            ...
            removeArrayObserver: function(target, opts) {
                ...
                var hasObservers = get(this, 'hasArrayObservers');
                if (hasObservers) Alto.propertyWillChange(this, 'hasArrayObservers');
                Alto.removeListener(this, '@array:before', target, willChange);
                Alto.removeListener(this, '@array:change', target, didChange);
                if (hasObservers) Alto.propertyDidChange(this, 'hasArrayObservers');
                return this;
            },
             ...
             */
            hasArrayObservers: Alto.computed(function() {
                return Alto.hasListeners(this, '@array:change') || Alto.hasListeners(this, '@array:before');
            }),
            ...
            arrayContentWillChange: function(startIdx, removeAmt, addAmt) {
                ...
                if (Alto.isWatching(this, '@each')) { get(this, '@each'); }

                Alto.sendEvent(this, '@array:before', [this, startIdx, removeAmt, addAmt]);
            ...
            arrayContentDidChange: function(startIdx, removeAmt, addAmt) {
                ...
                Alto.sendEvent(this, '@array:change', [this, startIdx, removeAmt, addAmt]);

                var length      = get(this, 'length'),
                    cachedFirst = cacheFor(this, 'firstObject'),
                    cachedLast  = cacheFor(this, 'lastObject');
                if (this.objectAt(0) !== cachedFirst) {
                    Alto.propertyWillChange(this, 'firstObject');
                    Alto.propertyDidChange(this, 'firstObject');
                }
                if (this.objectAt(length-1) !== cachedLast) {
                    Alto.propertyWillChange(this, 'lastObject');
                    Alto.propertyDidChange(this, 'lastObject');
                }
            ...
            /**
             ...
             @property @each
             */
            '@each': Alto.computed(function() {
                if (!this.__each) this.__each = new Alto.EachProxy(this);
                return this.__each;
            })
    ...
    (function() {
        var e_get = Alto.get,
            ...
            addObserver = Alto.addObserver,
            removeObserver = Alto.removeObserver,
            ComputedProperty = Alto.ComputedProperty,
            a_slice = [].slice,
            o_create = Alto.create,
            forEach = Alto.EnumerableUtils.forEach,
        ...
        function DependentArraysObserver(callbacks, cp, instanceMeta, context, propertyName, sugarMeta) {
            ...
            // a map of dependent array guids -> Alto.TrackedArray instances.  We use
        ...
        DependentArraysObserver.prototype = {
            ...
            teardownPropertyObservers: function (dependentKey, itemPropertyKeys) {
                ...
                trackedArray.apply(function (observerContexts, offset, operation) {
                    if (operation === Alto.TrackedArray.DELETE) { return; }
            ...
            resetTransformations: function (dependentKey, observerContexts) {
                this.trackedArraysByGuid[dependentKey] = new Alto.TrackedArray(observerContexts);
            },
            ...
            updateIndexes: function (trackedArray, array) {
                ...
                trackedArray.apply(function (observerContexts, offset, operation) {
                    ...
                    if (operation === Alto.TrackedArray.DELETE) { return; }
                    if (operation === Alto.TrackedArray.RETAIN && observerContexts.length === length && offset === 0) {
        ...
        function partiallyRecomputeFor(obj, dependentKey) {
            ...
            return Alto.Array.detect(value);
        }
                ...
                meta.dependentArraysObserver.suspendArrayObservers(function () {
                    forEach(cp._dependentKeys, function (dependentKey) {
                        Alto.assert(
                            ...
                            !(Alto.isArray(get(this, dependentKey)) && !Alto.Array.detect(get(this, dependentKey))));
        ...
        ReduceComputedProperty.prototype.property = function () {
            ...
            forEach(args, function (dependentKey) {
                if (doubleEachPropertyPattern.test(dependentKey)) {
                    throw new Alto.Error("Nested @each properties not supported: " + dependentKey);
    ...
    computed: Alto.reduceComputed('string', 'array', 'anotherArray.[]', {
      ...
      removedItem: removedItemCallback
         ...
         */
        Alto.reduceComputed = function (options) {
            var args;
            ...
            if (typeof options !== "object") {
                throw new Alto.Error("Reduce Computed Property declared without an options hash");
            }

            if (!('initialValue' in options)) {
                throw new Alto.Error("Reduce Computed Property declared without an initial value");
            }
    ...
    (function() {
        var ReduceComputedProperty = Alto.ReduceComputedProperty,
            a_slice = [].slice,
            o_create = Alto.create,
            forEach = Alto.EnumerableUtils.forEach;
            ...
            this.func = (function(reduceFunc) {
                return function (propertyName) {
                    if (!cp._hasInstanceMeta(this, propertyName)) {
                        ...
                        forEach(cp._dependentKeys, function(dependentKey) {
                            Alto.addObserver(this, dependentKey, function() {
...
// This is a stopgap to keep the reference counts correct with lazy CPs.
    ...
    return Alto.arrayComputed(dependentKey, options);
         ...
         @return {Alto.ComputedProperty}
         */
        Alto.arrayComputed = function (options) {
            var args;
            ...
            if (typeof options !== "object") {
                throw new Alto.Error("Array Computed Property declared without an options hash");
            }
    ...
    (function() {
        ...
        Alto.computed.sum = function(dependentKey){
            return Alto.reduceComputed(dependentKey, {
                initialValue: 0,
    ...
    age: 8
         ...
         */
        Alto.computed.min = function (dependentKey) {
            return Alto.reduceComputed(dependentKey, {
                initialValue: Infinity,
        ...
        /**
         ...
         ```javascript
         App.Hamster = Alto.Object.extend({
    excitingChores: Alto.computed.map('chores', function(chore) {
    ...
    age: 8
         ...
         */
        Alto.computed.mapBy = function(dependentKey, propertyKey) {
            var callback = function(item) { return get(item, propertyKey); };
            return Alto.computed.map(dependentKey + '.@each.' + propertyKey, callback);
        };
        ...
        /**
         ...
         @param propertyKey
         */
        Alto.computed.mapProperty = Alto.computed.mapBy;
        ...
        /**
         ...
         ```javascript
         App.Hamster = Alto.Object.extend({
    remainingChores: Alto.computed.filter('chores', function(chore) {
      return !chore.done;
    })
  });
    ...
    {name: 'write more unit tests', done: false}
         ...
         */
        Alto.computed.filter = function(dependentKey, callback) {
            var options = {
                initialize: function (array, changeMeta, instanceMeta) {
                    instanceMeta.filteredArrayIndexes = new Alto.SubArray();
                },
    ...
    {name: 'write more unit tests', done: false}
         ...
         @return {Alto.ComputedProperty} the filtered array
         */
        Alto.computed.filterBy = function(dependentKey, propertyKey, value) {
            var callback;
            ...
            return Alto.computed.filter(dependentKey + '.@each.' + propertyKey, callback);
        };
         ...
         */
        Alto.computed.filterProperty = Alto.computed.filterBy;
    ...
    'banana'
        ...
        Alto.computed.uniq = function() {
            ...
            return Alto.arrayComputed.apply(null, args);
        };
        ...
        /**
         ...
         unique elements from the dependent array
         */
        Alto.computed.union = Alto.computed.uniq;
    ...
    friendsInCommon: Alto.computed.intersect('adaFriends', 'charlesFriends')
         ...
         */
        Alto.computed.intersect = function () {
            ...
            args.push({
                ...
                addedItem: function(array, item, changeMeta, instanceMeta) {
                    ...
                    if (++itemCounts[itemGuid][dependentGuid] === 1 &&
                        numberOfDependentArrays === Alto.keys(itemCounts[itemGuid]).length) {
                ...
                removedItem: function(array, item, changeMeta, instanceMeta) {
                    ...
                    if (--itemCounts[itemGuid][dependentGuid] === 0) {
                        ...
                        numberOfArraysItemAppearsIn = Alto.keys(itemCounts[itemGuid]).length;
    ...
    'kale',
         ...
         dependent array
         */
        Alto.computed.setDiff = function (setAProperty, setBProperty) {
            if (arguments.length !== 2) {
                throw new Alto.Error("setDiff requires exactly two dependent arrays.");
            }
            return Alto.arrayComputed(setAProperty, setBProperty, {
                addedItem: function (array, item, changeMeta, instanceMeta) {
        ...
        SearchProxy = Alto.ObjectProxy.extend();
    ...
    {name: 'Release', priority: 1}
         ...
         */
        Alto.computed.sort = function (itemsKey, sortDefinition) {
            Alto.assert("Alto.computed.sort requires two arguments: an array key to sort and either a sort properties key or sort function", arguments.length === 2);
            ...
            if (typeof sortDefinition === 'function') {
                ...
                initFn = function (array, changeMeta, instanceMeta) {
                    function setupSortProperties() {
                        ...
                        Alto.assert("Cannot sort: '" + sortPropertiesKey + "' is not an array.", Alto.isArray(sortPropertyDefinitions));
                    ...
                    function updateSortPropertiesOnce() {
                        Alto.run.once(this, updateSortProperties, changeMeta.propertyName);
                    }
                    ...
                    Alto.addObserver(this, sortPropertiesKey, updateSortPropertiesOnce);
                    ...
                    instanceMeta.order = function (itemA, itemB) {
                        ...
                        for (var i = 0; i < this.sortProperties.length; ++i) {
                            ...
                            result = Alto.compare(get(itemA, sortProperty), isProxy ? itemB[sortProperty] : get(itemB, sortProperty));
            ...
            return Alto.arrayComputed(itemsKey, {
                initialize: initFn,
    ...
    (function() {
        Alto.RSVP = requireModule('rsvp');

        Alto.RSVP.onerrorDefault = function(error) {
            if (error instanceof Error) {
                if (Alto.testing) {
                    if (Alto.Test && Alto.Test.adapter) {
                        Alto.Test.adapter.exception(error);
                    } else {
                        throw error;
                    }
                } else {
                    Alto.Logger.error(error.stack);
                    Alto.assert(error, false);
                }
            }
        };

        Alto.RSVP.on('error', Alto.RSVP.onerrorDefault);
      ...
      valueObserver: function() {
        // Executes immediately after the "value" property changes
            ...
            Function.prototype.observesImmediately = function() {
                for (var i=0, l=arguments.length; i<l; i++) {
                    var arg = arguments[i];
                    Alto.assert("Immediate observers must observe internal properties only, not properties on other objects.", arg.indexOf('.') === -1);
                }
    ...
    (function() {
        ...
        /**
         ...
         @since Alto 0.9
         */
        Alto.Comparable = Alto.Mixin.create({
             ...
             */
            compare: Alto.required(Function)
    ...
    (function() {
        ...
        /**
         ...
         @since Alto 0.9
         */
        Alto.Copyable = Alto.Mixin.create({
             ...
             */
            frozenCopy: function() {
                if (Alto.Freezable && Alto.Freezable.detect(this)) {
                    return get(this, 'isFrozen') ? this : this.copy().freeze();
                } else {
                    throw new Alto.Error(Alto.String.fmt("%@ does not support freezing", [this]));
                }
            }
        });
    ...
    swapNames: function() {
      ...
      return this;
         ...
         @since Alto 0.9
         */
        Alto.Freezable = Alto.Mixin.create({
        ...
        Alto.FROZEN_ERROR = "Frozen object cannot be modified.";
    ...
    (function() {
        ...
        /**
         ...
         @uses Alto.Enumerable
         */
        Alto.MutableEnumerable = Alto.Mixin.create(Alto.Enumerable, {
             ...
             */
            addObject: Alto.required(Function),
            ...
            addObjects: function(objects) {
                Alto.beginPropertyChanges(this);
                forEach(objects, function(obj) { this.addObject(obj); }, this);
                Alto.endPropertyChanges(this);
                return this;
            },
            ...
            /**
             ...
             @return {Object} the passed object
             */
            removeObject: Alto.required(Function),
...
//
         ...
         */
        Alto.MutableArray = Alto.Mixin.create(Alto.Array, Alto.MutableEnumerable, {
            ...
            insertAt: function(idx, object) {
                if (idx > get(this, 'length')) throw new Alto.Error(OUT_OF_RANGE_EXCEPTION) ;
            ...
            removeAt: function(start, len) {
                if ('number' === typeof start) {
                    ...
                    if ((start < 0) || (start >= get(this, 'length'))) {
                        throw new Alto.Error(OUT_OF_RANGE_EXCEPTION);
                    }
            ...
            pushObjects: function(objects) {
                if (!(Alto.Enumerable.detect(objects) || Alto.isArray(objects))) {
    ...
    (function() {
        ...
        /**
         ...
         @extends Alto.Mixin
         */
        Alto.TargetActionSupport = Alto.Mixin.create({
            target: null,
            action: null,
            actionContext: null,

            targetObject: Alto.computed(function() {
                var target = get(this, 'target');

                if (Alto.typeOf(target) === "string") {
            ...
            actionContextObject: Alto.computed(function() {
                var actionContext = get(this, 'actionContext');

                if (Alto.typeOf(actionContext) === "string") {
    ...
    console.log('Offer one-time special');
         ...
         @namespace Alto
         */
        Alto.Evented = Alto.Mixin.create({
     ...
     // fired once the person has loaded
            ...
            on: function(name, target, method) {
                Alto.addListener(this, name, target, method);
                return this;
            },
            ...
            one: function(name, target, method) {
                ...
                Alto.addListener(this, name, target, method, true);
                return this;
            },
      ...
      console.log('person ate some ' + food);
            ...
            off: function(name, target, method) {
                Alto.removeListener(this, name, target, method);
                return this;
            },
             ...
             */
            has: function(name) {
                return Alto.hasListeners(this, name);
            }
        });
    ...
    (function() {
        ...
        if (Alto.FEATURES['ember-runtime-test-friendly-promises']) {
            ...
            RSVP.configure('async', function(callback, promise) {
                var async = !Alto.run.currentRunLoop;

                if (Alto.testing && async) { asyncStart(); }

                Alto.run.backburner.schedule('actions', function(){
        ...
        /**
         ...
         @namespace Alto
         */
        Alto.DeferredMixin = Alto.Mixin.create({
            /**
            ...
            _deferred: Alto.computed(function() {
    ...
    (function() {
        ...
        var get = Alto.get, typeOf = Alto.typeOf;
        ...
        /**
         ...
         @namespace Alto
         */
        Alto.ActionHandler = Alto.Mixin.create({
            ...
            willMergeMixin: function(props) {
                ...
                if (!props._actions) {
                    ...
                    if (typeOf(props.actions) === 'object') {
                        ...
                        Alto.deprecate('Action handlers contained in an `events` object are deprecated in favor of putting them in an `actions` object', false);
                        hashName = 'events';
                    }

                    if (hashName) {
                        props._actions = Alto.merge(props._actions || {}, props[hashName]);
                    }
        ...
        playMusic: function(track) {
          // ...
            ...
            send: function(actionName) {
                ...
                } else if (!Alto.FEATURES.isEnabled('ember-routing-drop-deprecated-action-style') && this.deprecatedSend && this.deprecatedSendHandles && this.deprecatedSendHandles(actionName)) {
                    Alto.warn("The current default is deprecated but will prefer to handle actions directly on the controller instead of a similarly named action in the actions hash. To turn off this deprecated feature set: Alto.FEATURES['ember-routing-drop-deprecated-action-style'] = true");
                ...
                if (target = get(this, 'target')) {
                    Alto.assert("The `target` for " + this + " (" + target + ") does not have a `send` method", typeof target.send === 'function');
    ...
    lastName: 'Penner'
         ...
         */
        Alto.PromiseProxyMixin = Alto.Mixin.create({
            /**
      ...
      promise: <thenable>
             ...
             @property promise
             */
            promise: Alto.computed(function(key, promise) {
                if (arguments.length === 2) {
                    return tap(this, promise);
                } else {
                    throw new Alto.Error("PromiseProxy's promise must be set");
                }
            }),
    ...
    (function() {
         ...
         */
        Alto.TrackedArray = function (items) {
        ...
        Alto.TrackedArray.RETAIN = RETAIN;
        Alto.TrackedArray.INSERT = INSERT;
        Alto.TrackedArray.DELETE = DELETE;

        Alto.TrackedArray.prototype = {
        ...
        Alto.SubArray.prototype = {
            ...
            removeItem: function(index) {
                ...
                this._findOperation(index, function (operation, operationIndex, rangeStart, rangeEnd, seenInSubArray) {
                    ...
                    throw new Alto.Error("Can't remove an item that has never been added.");
                });
    ...
    (function() {
        Alto.Container = requireModule('container')['default'];
        Alto.Container.set = Alto.set;
    ...
    (function() {
        Alto.Application = Alto.Namespace.extend();
    ...
    (function() {
      ...
      objectAtContent: function(idx) {
         ...
         */
        Alto.ArrayProxy = Alto.Object.extend(Alto.MutableArray, {
             ...
             */
            arrangedContent: Alto.computed.alias('content'),
             ...
             */
            _contentWillChange: Alto.beforeObserver('content', function() {
            ...
            contentArrayWillChange: Alto.K,
            contentArrayDidChange: Alto.K,
            ...
            _setupContent: function() {
                ...
                if (content) {
                    Alto.assert(Alto.String.fmt('ArrayProxy expects an Array or ' +
                        'Alto.ArrayProxy, but you passed %@', [typeof content]),
                        Alto.isArray(content) || content.isDestroyed);
            ...
            _arrangedContentWillChange: Alto.beforeObserver('arrangedContent', function() {
            ...
            _arrangedContentDidChange: Alto.observer('arrangedContent', function() {
                var arrangedContent = get(this, 'arrangedContent'),
                    len = arrangedContent ? get(arrangedContent, 'length') : 0;
            ...
            _setupArrangedContent: function() {
                ...
                if (arrangedContent) {
                    Alto.assert(Alto.String.fmt('ArrayProxy expects an Array or ' +
                        ...
                        Alto.isArray(arrangedContent) || arrangedContent.isDestroyed);
            ...
            arrangedContentWillChange: Alto.K,
            arrangedContentDidChange: Alto.K,

            objectAt: function(idx) {
                return get(this, 'content') && this.objectAtContent(idx);
            },

            length: Alto.computed(function() {
                var arrangedContent = get(this, 'arrangedContent');
            ...
            replace: function() {
                if (get(this, 'arrangedContent') === get(this, 'content')) {
                    ...
                    throw new Alto.Error("Using replace on an arranged ArrayProxy is not allowed.");
                }
            },

            _insertAt: function(idx, object) {
                if (idx > get(this, 'content.length')) throw new Alto.Error(OUT_OF_RANGE_EXCEPTION);
            ...
            insertAt: function(idx, object) {
                if (get(this, 'arrangedContent') === get(this, 'content')) {
                    ...
                    throw new Alto.Error("Using insertAt on an arranged ArrayProxy is not allowed.");
                }
            },
    ...
    (function() {
        ...
        var set = Alto.set, get = Alto.get, guidFor = Alto.guidFor;
        var forEach = Alto.EnumerableUtils.forEach,
            indexOf = Alto.ArrayPolyfills.indexOf;

        var EachArray = Alto.Object.extend(Alto.Array, {
        ...
        function addObserverForContentKey(content, keyName, proxy, idx, loc) {
            ...
            while(--loc>=idx) {
                ...
                if (item) {
                    Alto.assert('When using @each to observe the array ' + content + ', the array must return an object', Alto.typeOf(item) === 'instance' || Alto.typeOf(item) === 'object');
                    Alto.addBeforeObserver(item, keyName, proxy, 'contentKeyWillChange');
                    Alto.addObserver(item, keyName, proxy, 'contentKeyDidChange');
        ...
        function removeObserverForContentKey(content, keyName, proxy, idx, loc) {
            ...
            while(--loc>=idx) {
                var item = content.objectAt(loc);
                if (item) {
                    Alto.removeBeforeObserver(item, keyName, proxy, 'contentKeyWillChange');
                    Alto.removeObserver(item, keyName, proxy, 'contentKeyDidChange');
        ...
        /**
         ...
         @extends Alto.Object
         */
        Alto.EachProxy = Alto.Object.extend({
            ...
            init: function(content) {
                ...
                // added
                forEach(Alto.watchedEvents(this), function(eventName) {
            ...
            unknownProperty: function(keyName, value) {
                ...
                Alto.defineProperty(this, keyName, null, ret);
            ...
            arrayWillChange: function(content, idx, removedCnt, addedCnt) {
                ...
                for(key in keys) {
                    ...
                    Alto.propertyWillChange(this, key);
                }

                Alto.propertyWillChange(this._content, '@each');
                Alto.endPropertyChanges(this);
            },
            ...
            arrayDidChange: function(content, idx, removedCnt, addedCnt) {
                ...
                Alto.changeProperties(function() {
                    for(var key in keys) {
                        ...
                        Alto.propertyDidChange(this, key);
                    }

                    Alto.propertyDidChange(this._content, '@each');
                }, this);
            },
    ...
    (function() {
        ...
        var get = Alto.get, set = Alto.set, replace = Alto.EnumerableUtils._replace;

// Add Alto.Array to Array.prototype. Remove methods with native
// implementations and supply some more optimized versions of generic methods
// because they are so common.
        var NativeArray = Alto.Mixin.create(Alto.MutableArray, Alto.Observable, Alto.Copyable, {
            ...
            replace: function(idx, amt, objects) {
                ...
                if (this.isFrozen) throw Alto.FROZEN_ERROR;
            ...
            copy: function(deep) {
                if (deep) {
                    return this.map(function(item) { return Alto.copy(item, true); });
                }
...
// Remove any methods implemented natively so we don't override them
        var ignore = ['length'];
        Alto.EnumerableUtils.forEach(NativeArray.keys(), function(methodName) {
    ...
    init: function() {
      ...
      if (!this.get('content')) {
        ...
        Alto.A = function(arr) {
            ...
            return Alto.Array.detect(arr) ? arr : Alto.NativeArray.apply(arr);
        };
    ...
    Alto.NativeArray.activate();
         ...
         @return {void}
         */
        Alto.NativeArray.activate = function() {
            NativeArray.apply(Array.prototype);

            Alto.A = function(arr) { return arr || []; };
        };
    ...
    (function() {
        ...
        var get = Alto.get, set = Alto.set, guidFor = Alto.guidFor, isNone = Alto.isNone, fmt = Alto.String.fmt;
        ...
        /**
         ...
         @since Alto 0.9
         */
        Alto.Set = Alto.CoreObject.extend(Alto.MutableEnumerable, Alto.Copyable, Alto.Freezable,
            {
                 ...
                 */
                clear: function() {
                    if (this.isFrozen) { throw new Alto.Error(Alto.FROZEN_ERROR); }
                ...
                isEqual: function(obj) {
                    // fail fast
                    if (!Alto.Enumerable.detect(obj)) return false;
                 ...
                 */
                add: Alto.aliasMethod('addObject'),
                ...
                /**
                 ...
                 @return {Alto.Set} The set itself.
                 */
                remove: Alto.aliasMethod('removeObject'),
                 ...
                 */
                pop: function() {
                    if (get(this, 'isFrozen')) throw new Alto.Error(Alto.FROZEN_ERROR);
                ...
                /**
                 ...
                 @return {Alto.Set} The set itself.
                 */
                push: Alto.aliasMethod('addObject'),
                ...
                /**
                 ...
                 @return {Alto.Set} The set itself.
                 */
                addEach: Alto.aliasMethod('addObjects'),
                ...
                /**
                 ...
                 @return {Alto.Set} The set itself.
                 */
                removeEach: Alto.aliasMethod('removeObjects'),
                ...
                removeObject: function(obj) {
                    ...
                    if (idx>=0 && idx<len && (this[idx] === obj)) {
                        ...
                        if (isFirst) { Alto.propertyWillChange(this, 'firstObject'); }
                        if (isLast)  { Alto.propertyWillChange(this, 'lastObject'); }
                        ...
                        if (isFirst) { Alto.propertyDidChange(this, 'firstObject'); }
                        if (isLast)  { Alto.propertyDidChange(this, 'lastObject'); }
    ...
    (function() {
        var DeferredMixin = Alto.DeferredMixin, // mixins/deferred
            get = Alto.get;

        var Deferred = Alto.Object.extend(DeferredMixin);
    ...
    (function() {
        ...
        var forEach = Alto.ArrayPolyfills.forEach;
    ...
    hbars.registerHelper(...);
         ...
         */
        Alto.onLoad = function(name, callback) {
            var object;
         ...
         */
        Alto.runLoadHooks = function(name, object) {
            loaded[name] = object;
    ...
    (function() {
         ...
         */
        Alto.ControllerMixin = Alto.Mixin.create(Alto.ActionHandler, {
            ...
            /**
             ...
             instantiated by `Alto.Application#initialize`.
            ...
            model: Alto.computed.alias('content'),
            ...
            deprecatedSend: function(actionName) {
                ...
                Alto.assert('' + this + " has the action " + actionName + " but it is not a function", typeof this[actionName] === 'function');
                Alto.deprecate('Action handlers implemented directly on controllers are deprecated in favor of action handlers on an `actions` object ( action: `' + actionName + '` on ' + this + ')', false);
         ...
         */
        Alto.Controller = Alto.Object.extend(Alto.ControllerMixin);
    ...
    (function() {
        ...
        var get = Alto.get, set = Alto.set, forEach = Alto.EnumerableUtils.forEach;
    ...
    sortAscending: true
         ...
         @uses Alto.MutableEnumerable
         */
        Alto.SortableMixin = Alto.Mixin.create(Alto.MutableEnumerable, {
      ...
      return x < y ? -1 : 1;
                ...
                if (content && sortProperties) {
                    forEach(content, function(item) {
                        forEach(sortProperties, function(sortProperty) {
                            Alto.removeObserver(item, sortProperty, this, 'contentItemSortPropertyDidChange');
                        }, this);
                    }, this);
                }
            ...
            isSorted: Alto.computed.bool('sortProperties'),
            ...
            arrangedContent: Alto.computed('content', 'sortProperties.@each', function(key, value) {
                var content = get(this, 'content'),
                ...
                if (content && isSorted) {
                    ...
                    forEach(content, function(item) {
                        forEach(sortProperties, function(sortProperty) {
                            Alto.addObserver(item, sortProperty, this, 'contentItemSortPropertyDidChange');
                        }, this);
                    }, this);
            ...
            sortAscendingWillChange: Alto.beforeObserver('sortAscending', function() {
                this._lastSortAscending = get(this, 'sortAscending');
            }),

            sortAscendingDidChange: Alto.observer('sortAscending', function() {
    ...
    (function() {
        ...
        var get = Alto.get, set = Alto.set, forEach = Alto.EnumerableUtils.forEach,
            replace = Alto.EnumerableUtils.replace;
        ...
        /**
         ...
         request. Create an `Alto.ArrayController` and set its `content` property:
    ...
    lookupItemController: function( object ) {
      if (object.get('isSpecial')) {
        ...
        Alto.ArrayController = Alto.ArrayProxy.extend(Alto.ControllerMixin,
            Alto.SortableMixin, {
        ...
        if (object.get('isSpecial')) {
          ...
          return "regular"; // use App.RegularController
                ...
                init: function() {
                    ...
                    this.set('_subControllers', Alto.A());
                },

                content: Alto.computed(function () {
                    return Alto.A();
                }),
                ...
                controllerAt: function(idx, object, controllerClass) {
                    ...
                    if (!container.has(fullName)) {
                        throw new Alto.Error('Could not resolve itemController: "' + controllerClass + '"');
                    }
    ...
    (function() {
        ...
        /**
         ...
         @uses Alto.ControllerMixin
         **/
        Alto.ObjectController = Alto.ObjectProxy.extend(Alto.ControllerMixin);
